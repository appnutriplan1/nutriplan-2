...
function manejarSolicitud(e) {
  try {
    ...
    switch (accion) {
      ...
      case 'login-suscriptor':
        return responder(loginSuscriptor(params.codigo_acceso));
      case 'planes':
      ...
      case 'admin_suscriptores':
        return responder(adminListarSuscriptores(params.admin_password));
      case 'admin_generar_codigo_suscriptor':
        return responder(adminGenerarCodigoSuscriptor(params.admin_password, params.email, params.nombre));
      case 'admin_actualizar_suscriptor':
        return responder(adminActualizarSuscriptor(params.admin_password, params.suscriptor_id, params.estado, params.renovar));
      // Operación interna de Salud Laboral
      case 'laboral_resumen':
        return responder(laboralResumen(params.admin_password));
      case 'laboral_guardar_empresa':
        return responder(laboralGuardarEmpresa(params.admin_password, params.empresa));
      case 'laboral_guardar_jornada':
        return responder(laboralGuardarJornada(params.admin_password, params.jornada));
      case 'laboral_guardar_evaluacion':
        return responder(laboralGuardarEvaluacion(params.admin_password, params.evaluacion));
      case 'laboral_eliminar_evaluacion':
        return responder(laboralEliminarEvaluacion(params.admin_password, params.evaluacion_id));
      case 'laboral_guardar_checklist':
        return responder(laboralGuardarChecklist(params.admin_password, params.jornada_id, params.checklist_json));
      case 'crear_suscriptor':
        if (!esAdminSeguro(params.admin_password)) return responder({ ok: false, error: 'NO_AUTORIZADO' });
        return responder(crearSuscriptor(params.codigo, params.email, params.nombre));
      case 'generar_codigo_suscriptor':
        if (!esAdminSeguro(params.admin_password)) return responder({ ok: false, error: 'NO_AUTORIZADO' });
        return responder(generarCodigoSuscriptor(params.email, params.nombre));
      case 'migrar_suscriptores':
        if (!esAdminSeguro(params.admin_password)) return responder({ ok: false, error: 'NO_AUTORIZADO' });
        return responder(migrarSuscriptoresDesdePacientes());
      default:
